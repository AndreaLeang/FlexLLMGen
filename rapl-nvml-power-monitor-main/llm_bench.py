...
class LLMPowerBench:
    ...
    def run(
        ...
        while True:
                ...
                for i in range(self.model.execute_gen_len):
                    ...
                    for j in range(num_layers):
                        ...
                        for each_iter in range(n_iters_layer):
                            ...
                            self.model.load_hidden(i, j, 0, repeating)
                            self.model.compute_layer(i, j, 0, repeating)
                            self.model.store_cache(i, j-1, 0, repeating)
                            self.model.store_hidden(i, j, 0, repeating)
                            self.model.sync()
                            li1 = len(mon.samples)
            ...
            #                   self.model.store_hidden(i, j, k-1, repeating)
            #                   self.model.load_hidden(i, j, k+1, repeating)
            #                   self.model.compute_layer(i, j, k, repeating)
            #                   self.model.store_cache(i, j, k-1, repeating)
            #                   self.model.sync()
            #                   li1 = len(mon.samples)
            #                   lt1 = time.perf_counter()
