"""
Usage:
python3 -m flexllmgen.flex_opt_kvpr --model facebook/opt-1.3b --gpu-batch-size 32 --percent 100 0 100 0 100 0
"""
...
class SelfAttention:
    def __init__(self, config, env, policy, layer_id, recompute_len, cpu_gpu_compute):
        self.config = config
        self.env = env
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        # Load Values for Hidden Compute (computing KV Cache)
        if self.recompute_len > 0 and i > 0:
            ...
            print(f"k_compute after reshape: {k_compute.shape}")
            print(f"v_compute after reshape: {v_compute.shape}")
            
            k_compute_new = k_compute.permute(1, 0, 2, 3).reshape(compute_s, b * n_head, head_dim)
            v_compute_new = v_compute.permute(1, 0, 2, 3).reshape(compute_s, b * n_head, head_dim)

            print(f"k_compute after reshape2: {k_compute.shape}")
            print(f"v_compute after reshape2: {v_compute.shape}")
            if self.cpu_gpu_compute:
        ...
        if i == 0:  # prefill
            ...
            # possible reduction in latency: changing gpu and cpu order? 
            # KVPR: Also calculatin MHA on cpu: 
            if self.cpu_gpu_compute:
                # decompress weights
            ...
            if not repeating:
                ...
                (k_cache, donate[12]), (v_cache, donate[13]) = cache_read_buf.val
 
            # KVPR: partial calculate MHA on GPU 
            if self.cpu_gpu_compute:
                if self.recompute_len > 0:
                    if self.policy.compress_cache:
...
class TransformerLayer:
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        self.attention.forward(hidden, cache_read_buf, read_buf1, attention_mask,
                               cache_write_buf, i, k, hidden_compute, cpu_cache_read_buf, repeating)
        self.mlp.forward(hidden, None, read_buf2, attention_mask, None, i, k, repeating)
...
class OptLM:
    ...
    def load_cache(self, i, j, k, overlap=True, KVLoadTimer=None):
        ...
        if overlap:
            with torch.cuda.stream(self.load_cache_stream):
                self.layers[j].load_cache(self.cache_home[j][k], self.cache_read_buf[j][k], i, self.cpu_cache_read_buf[j][k], KVLoadTimer=KVLoadTimer)
        else:
            self.layers[j].load_cache(self.cache_home[j][k], self.cache_read_buf[j][k], i, self.cpu_cache_read_buf[j][k], KVLoadTimer=KVLoadTimer)
    ...
    def generation_loop_debug_normal(self):
        ...
        for stage in ["prefill", "decoding"]:
            for func in ["load_cache", "store_cache", "compute_layer" "load_hidden_compute"]:
                name = func + "_" + stage
...
def run_flexllmgen(args):
    ...
    print("init weight...")
    try:
        model = OptLM(opt_config, env, args.path, policy, recompute_len, args.cpu_gpu_compute)
    except: 
        print(f"error in init weight, do not run")
        return None
