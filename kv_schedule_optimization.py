...
def get_available_offloadings(opt_config, hardware_config, batch_sizes):
    ...
    for each_batch_size in batch_sizes:
        for each_possible_offloading in batch_size_to_distinct_offloadings[each_batch_size]:
            total_kv_cache_bytes = opt_config.cache_bytes(batch_size, seq_len) # Bytes (Toal KV Cache per forward pass) (seq_len = prompt_len+gen_len)
            total_hidden_bytes = opt_config.hidden_bytes(batch_size, seq_len) # Bytes (Total Hidden State per forward pass) (seq_len = prompt_len+gen_len)
...
def strategy_prediction(model, num_of_prompts, prompt_len, gen_len, hardware_config, recomp_len, offload_percent, batch_size, num_batches):
    ...
    input_output_latency = layer_prediction(model, 1, "input", batch_size, num_batches, offload_percent, recomp_len, prompt_len, gen_len) + (num_batches-1)*layer_prediction(model, 0, "input", batch_size, num_batches, offload_percent, recomp_len, prompt_len, gen_len) +(num_batches)*layer_prediction(model, 1, "output", batch_size, num_batches, offload_percent, recomp_len, prompt_len, gen_len) + layer_prediction(model, 0, "output", batch_size, num_batches, offload_percent, recomp_len, prompt_len, gen_len)
